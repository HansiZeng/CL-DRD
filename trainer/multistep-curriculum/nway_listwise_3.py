...
from evaluation.reranking_evaluator import RerankingEvaluator
...
def get_args():
    ...
    parser.add_argument("--training_path", default="/mnt/nfs/scratch1/hzeng/datasets/msmarco-passage/corase_to_fine_grained/20relT_10neg.train.json")
    ...
    parser.add_argument("--model_checkpoint", default="/mnt/nfs/scratch1/hzeng/my-msmarco-passage/experiments/multistep-curriculum/experiment_01-15_172705/models/checkpoint_210000.pth.tar")
    ...
    parser.add_argument("--in_batch_loss", default=False)
    parser.add_argument("--all_in_batch_neg", default=False)
    ...
    if args.local_rank < 1:
        ...
        args.run_folder = os.path.join(EXPERIMENT_FOLDER, args.run_folder+"_"+time.strftime("%m-%d_%H%M%S", time.localtime()))
        if not os.path.exists(args.run_folder):
            os.mkdir(args.run_folder)
    ...
    if args.distributed:
        ...
        if args.label_mode == "5":
            train_dataset = NwayDataset.create_from_20relT_10neg_file(args.queries_path, args.collection_path, args.training_path, tokenizer, 
                                                    ...
                                                    label_mode="5", rank=args.local_rank, nranks=args.nranks)
